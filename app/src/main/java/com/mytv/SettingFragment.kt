...
import com.github.mytv.databinding.SettingBinding
...
class SettingFragment : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        binding.btnPlayerType.textSize = textSize
        binding.btnIjkDecoder.textSize = textSize
        binding.btnExoDecoder.textSize = textSize
    ...
    private fun setupPlayerConfigButtons() {
        refreshPlayerConfigButtonTexts()

        // 播放器内核：exo <-> ijk
        binding.btnPlayerType.setOnClickListener {
            SP.playerType = if (SP.playerType == "exo") "ijk" else "exo"
            refreshPlayerConfigButtonTexts()
            // SP.playerType setter 内部会触发 OnSharedPreferenceChanged，
            // MainActivity 会调 playerFragment.onPlayerConfigChanged()，无需重复
            (activity as MainActivity).settingDelayHide()
        }

        // ijk 解码方式：auto -> hard -> soft -> auto
        binding.btnIjkDecoder.setOnClickListener {
        ...
        // exo 解码方式：default <-> software
        binding.btnExoDecoder.setOnClickListener {
    ...
    private fun refreshPlayerConfigButtonTexts() {
        val playerTypeText = getString(R.string.title_player_type) + ": " +
            getString(if (SP.playerType == "ijk") R.string.player_type_ijk else R.string.player_type_exo)
        binding.btnPlayerType.text = playerTypeText

        val ijkDecoderText = getString(R.string.title_ijk_decoder) + ": " +
            when (SP.ijkDecoder) {
                "hard" -> getString(R.string.decoder_hard)
                "soft" -> getString(R.string.decoder_soft)
                else -> getString(R.string.decoder_auto)
            }
        binding.btnIjkDecoder.text = ijkDecoderText

        val exoDecoderText = getString(R.string.title_exo_decoder) + ": " +
            if (SP.exoDecoder == "software")
                getString(R.string.decoder_software)
            else
                getString(R.string.decoder_default)
        binding.btnExoDecoder.text = exoDecoderText
    }
